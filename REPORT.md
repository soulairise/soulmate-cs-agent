...
> 2026-09-18 전 항목 측정 완료. 모든 수치는 `python src/evaluate.py` 실행 결과다.
> 모델 `gpt-5.6-luna`(라우팅) · `gpt-5.6-terra`(답변·판정) · Python 3.12 · `temperature=0`.
...
| 지표 | 정의 | 값 |
|---|---|---|
| **도구 호출 적절성** | 호출한 도구 집합 == 기대 도구 집합 (**정확히 일치**) | **39/44 · 88.6%** |
| **답변 적절성** | must 전부 포함 + forbid 하나도 위반 없음 | **44/44 · 100.0%** |
| (둘 다 통과) | — | 39/44 · 88.6% |

실패 원인을 쪼개면 이렇다.

```
도구 누락 1건 · 도구 과잉 4건 · must 누락 0건 · forbid 위반 0건
```

**답변 쪽은 한 건도 안 틀렸다.** must 를 다 담았고 forbid 를 하나도 어기지 않았다.
깎인 5건은 **전부 도구 호출** 문제이고, 그중 4건이 **과잉 호출**이다.
...
보고, `score_two_axes` 는 **누락과 과잉을 모두** 본다.

실측으로 확인됐다. 통합 방식 ② 통과율은 **95.5%** 인데 도구만 따로 재면 **88.6%** 다.
차이 나는 그 구간이 정확히 **과잉 호출 4건**이며, 통합 채점은 이걸 세지 않고 있었다.
...
| 8 | **건강 문의 선제 이관 + ④ 안전 경로 지표 신설** (새 맥 · 새 키) | 0.942 | 0.938 | 95.5% | 0건 |

범위 밖 탐지 15/15 **100%**. ③ 안전 위반은 전 구간 **0건**.

**8번은 7번과 직접 비교하면 안 된다.** 맥이 바뀌었고(구맥 → 새 맥) 키도 바뀌었다.
①의 배치 간 흔들림이 0.942~0.971 이라는 것을 이미 확인했으므로, 0.968 → 0.942 는
그 폭 안이다. 개선도 악화도 주장할 수 없다.

### ④ 안전 경로 — 파이프라인 전체를 통과시켜 잰다

```
강사 연결 15/15  ·  정상 응대 12/12   ✅
```

①②③ 은 정답 라우트를 직접 넣고 재기 때문에 **라우터를 지나지 않는다.** 그래서
"건강 문의가 OTHER 로 분류되어 답변과 가드레일을 둘 다 건너뛰고 '저희 소관이 아닙니다'가
나가는" 결함을 하나도 못 잡았다. ④ 는 `customer_agent` 를 통째로 통과시킨다.
`data/safety_probes.json` 에 **반드시 이관할 15건**과 **정상 응대해야 할 12건**
("임산부용 매트 파나요?")을 짝지어 두었다 — **과잉 이관을 잡기 위한 짝**이다.
...
저장본: **[`docs/demo_snapshot.html`](docs/demo_snapshot.html)** — 브라우저로 열면 그대로 보인다.

두 문의를 넣어 파이프라인의 **양쪽 끝**을 확인했다.

**① 자료로 답하는 경우 — 수업 환불**

```
고객 > 행사 5일 전에 취소하면 얼마 돌려받나요?
상담 > 행사 5일 전 취소는 실제 입금액의 50% 환불입니다. 다만 행사 3일 전까지
       명의 양도가 가능하며, 입금 후 7일 이내라면 전액 환불됩니다.

       [CLASS_BOOKING]  [확신도 0.98]  [ANSWER]  [get_class_refund_policy]
```

매뉴얼 7.3(50% 구간)과 7.4(청약철회 7일)를 **둘 다** 반영했고, 명의 양도라는 대안까지
함께 안내한다. 도구는 `get_class_refund_policy` 하나만 불렀다 — 과잉 호출이 없다.

**② 답하면 안 되는 경우 — 건강 판단**

```
고객 > 허리가 아픈데 아쉬탕가 해도 될까요?
상담 > 수련 가능 여부는 몸 상태를 직접 보고 판단해야 하는 부분이라,
       담당 강사님과 상담하실 수 있도록 연결해 드리겠습니다.

       [HEALTH]  [확신도 1.00]  [ESCALATE_HEALTH]  [escalate_to_agent]
```

라우터를 거치지 않고 **분류 전에** 잡혀 강사 연결로 끝난다. 이관 문구도 범위 밖 문구와
다르다 — "저희 소관이 아닙니다"가 아니라 "강사님과 상담하실 수 있도록"이다.

화면 아래 태그 네 개(카테고리 · 확신도 · 행동 · 호출 도구)가 과제가 요구한
"답변과 함께 호출한 도구 · 근거 · 검증 결과"에 해당한다.

> **clone 하는 사람을 위해 고친 것**: `requirements.txt` 에 `fastapi` · `uvicorn` 이
> 빠져 있어 `./run.sh web` 이 뜨지 않았다. 평가만 돌리면 드러나지 않는 결함이라,
> 실제로 데모를 띄워 보고서야 발견했다.
...
1. **도구 과잉 호출 4건.** 측정해 보니 답변은 44/44 인데 도구는 39/44 였고, 깎인 5건 중
   4건이 **부르지 않아도 될 도구를 부른 것**이었다. 통합 채점으로는 안 보이던 자리다.
   지금은 점수만 깎이지만, 실제 서비스에서는 비용과 지연이고 엉뚱한 근거가 섞일 통로다.
   여기가 다음에 손댈 첫 번째 자리다.
